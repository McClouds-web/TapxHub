...
const getInitialTasks = (): Task[] => {
  const now = new Date();
  return [
    { id: 1, title: "Finalize brand guidelines document", project: "Tapx Client", dueDate: addDays(now, 2), status: "in_progress", priority: "high" },
    { id: 2, title: "Set up Google Analytics tracking", project: "Tapx Client", dueDate: addDays(now, 3), status: "todo", priority: "high" },
    { id: 3, title: "Design homepage wireframe", project: "Tapx Client", dueDate: addDays(now, 4), status: "todo", priority: "medium" },
    { id: 4, title: "Write email sequence copy", project: "Apex Digital", dueDate: addDays(now, 5), status: "in_progress", priority: "medium" },
    { id: 5, title: "Review competitor analysis", project: "Greenfield Corp", dueDate: addDays(now, 1), status: "done", priority: "low" },
    { id: 6, title: "Create social media content calendar", project: "Bloom Studio", dueDate: addDays(now, 6), status: "todo", priority: "low" },
    { id: 7, title: "Optimize landing page speed", project: "NovaTech", dueDate: addDays(now, 2), status: "in_progress", priority: "high" },
    { id: 8, title: "Submit final SEO report", project: "Wildflower Co", dueDate: addDays(now, -2), status: "done", priority: "medium" },
  ];
};
...
export default function Tasks() {
  ...
  const allTasks = getInitialTasks();

  const filteredTasks = allTasks.filter(t => {
    const roleMatches = isAdmin || t.project === user?.name || t.project === "Tapx Client";
              ...
              >
                <div className="flex items-center gap-4 flex-1">
                  <div className={cn(
                    ...
                    <Icon className={cn("h-6 w-6", statusColors[task.status])} />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <p className="text-xs font-semibold text-muted-foreground flex items-center gap-1">
                        <Tag className="h-3 w-3" /> {task.project}
                      </p>
                      <p className="text-xs font-semibold text-muted-foreground flex items-center gap-1">
                        <Calendar className="h-3 w-3" /> Due {format(task.dueDate, "MMM d")}
                      </p>
                    </div>
                  </div>
                </div>
