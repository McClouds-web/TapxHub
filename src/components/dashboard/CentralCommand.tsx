...
export const CentralCommand: React.FC = () => {
  ...
  if (isLoading) return <div className="animate-pulse bg-[#F8FAFC] h-full rounded-xl" />;
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* KPI: Sentiment Avg */}
        <div className="glass-card p-3 border-none bg-indigo-500 text-white shadow-xl shadow-indigo-500/20">
        ...
        <div className="glass-card p-3 border-none bg-[#0F1E3D] text-white shadow-xl shadow-[#0F1E3D]/10">
          <div className="flex justify-between items-start mb-4">
        ...
        <div className="glass-card p-3 flex flex-col justify-center items-center gap-3 border-dashed border-[#0F1E3D]/20 hover:border-indigo-500 transition-all cursor-pointer group">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4 min-h-0">
        ...
        <div className="glass-card p-3 flex flex-col h-full">
           <h3 className="text-[10px] font-black uppercase tracking-widest text-[#0F1E3D] mb-4">Client Health System</h3>
           <div className="space-y-4 flex-1 overflow-y-auto pr-1">
             {MOCK_HEALTH.map((client) => (
               <div key={client.id} className="p-3 rounded-xl bg-[#F8FAFC] border border-[#0F1E3D]/5 hover:border-indigo-500/30 transition-all group">
                 <div className="flex items-center justify-between mb-3">
                    <span className="text-[10px] font-black text-[#0F1E3D]">{client.name}</span>
                    <div className={cn(
                      "text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter",
                      client.status === 'growth' ? "bg-blue-500/10 text-blue-600" :
                 ...
                 <div className="w-full bg-[#0F1E3D]/5 h-2 rounded-full overflow-hidden">
                    <div 
                      className={cn(
                        ...
                        client.sentiment > 80 ? "bg-blue-500" : client.sentiment > 50 ? "bg-amber-500" : "bg-rose-500"
                      )} 
        ...
        {/* Opportunity Radar */}
        <div className="glass-card p-3 flex flex-col h-full bg-[#F8FAFC] border-none">
           <h3 className="text-[10px] font-black uppercase tracking-widest text-[#0F1E3D] mb-4">Opportunity Radar</h3>
           <div className="space-y-3 flex-1 overflow-y-auto pr-1">
              {opportunities.map((opp, i) => (
                <div key={i} className="flex items-start gap-4 p-3 bg-white rounded-xl shadow-sm border border-[#0F1E3D]/5">
                   <div className="w-10 h-10 rounded-xl bg-blue-500/10 flex items-center justify-center shrink-0">
                      <TrendingUp className="w-5 h-5 text-blue-600" />
                   </div>
                   <div>
                      <p className="text-[10px] font-black text-[#0F1E3D]">{opp.title}</p>
                      <p className="text-[10px] text-[#0F1E3D]/60 font-medium mt-1 leading-relaxed">
                        {opp.description}
                      </p>
                      <div className="mt-3 flex items-center gap-2">
                         <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest bg-blue-50 px-2 py-0.5 rounded-md">{opp.probability}</span>
                         <span className="text-[10px] font-black text-[#0F1E3D]/40 uppercase tracking-widest">Est. {opp.value}</span>
                      </div>
                   </div>
                </div>
              ))}
