...
export function ClientPerformanceChart() {
  return (
    <motion.div
      ...
      className="flex flex-col h-full bg-[#f8fafc]/50 p-5 sm:p-6"
      style={{
    ...
    >
      <div className="flex items-center justify-between mb-5 shrink-0 px-1">
        <div>
          <h3 className="text-xl font-extrabold text-[var(--brand-primary)] tracking-tight">Project Health View</h3>
        ...
        {kpiConfig.map((kpi, i) => {
          ...
          return (
            <motion.div
              ...
              className="flex-1 flex items-center justify-between p-4 bg-white border border-[#0F1E3D]/5 rounded-2xl hover:border-[#0F1E3D]/15 hover:shadow-md transition-all group cursor-pointer"
            >
              {/* Left Logo + Titles */}
              <div className="flex items-center gap-4 w-1/3 min-w-0">
                ...
                <div className="min-w-0 truncate">
                  ...
                  <p className="text-xl sm:text-2xl font-extrabold text-[var(--brand-primary)] tracking-tight truncate">{kpi.value}</p>
                </div>
              </div>
              ...
              <div className="flex flex-col items-end justify-center w-1/4 sm:w-1/3 min-w-0 pl-2">
                <span className={cn(
                  ...
                  kpi.isPositive ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-red-50 text-red-600 border border-red-100"
                )}>
                  {kpi.isPositive ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                  {kpi.delta}
                </span>
                <span className="text-[9px] font-bold text-[var(--brand-primary)]/30 mt-1.5 uppercase tracking-widest hidden sm:block">vs Last Month</span>
              </div>
            </motion.div>
          );
        })}
