  ...
  return (
    <div ref={ref} className="p-12 bg-white text-slate-900 max-w-4xl mx-auto print:p-8" id="invoice-printable">
      ...
      <div className="flex justify-between items-start mb-12">
        <div>
          <img src="/logo.png" alt="TapxMedia" className="h-12 w-auto mb-6" />
          <h1 className="text-4xl font-black uppercase tracking-tighter text-[#0F1E3D]">INVOICE</h1>
        </div>
        <div className="text-right">
          <p className="font-bold text-lg">TapxMedia Agency</p>
          <p className="text-sm text-slate-500">tapiwa@tapxmedia.com</p>
            ...
            {invoice.items && invoice.items.length > 0 ? (
              invoice.items.map((item, idx) => (
                <tr key={idx} className="border-b border-slate-50">
                  ...
                  <td className="py-6 text-right font-bold">${item.amount.toLocaleString()}</td>
                </tr>
              ))
            ) : (
              <tr className="border-b border-slate-50">
                <td className="py-6 font-bold">Standard Agency Services</td>
      ...
      <div className="flex justify-end mb-12">
        <div className="w-64 space-y-4">
          ...
          <div className="flex justify-between pt-4 border-t-2 border-slate-100">
            <span className="text-lg font-black uppercase tracking-widest text-[#0F1E3D]">TOTAL</span>
            <span className="text-2xl font-black text-[#0F1E3D]">${total.toLocaleString()}</span>
          </div>
        </div>
      </div>
      ...
      <div className="pt-12 border-t border-slate-100 text-center">
        <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-2">Thank you for your business!</p>
