...
export function ClientNavbar({ mobileOpen = false, onMobileClose }: ClientNavbarProps) {
  ...
  return (
    ...
    >
      {/* Logo Area */}
      <div className="flex flex-col items-center justify-center py-8 px-4 border-b border-[#0F1E3D]/5 relative">
        <Link to="/portal" className="flex items-center justify-center w-full">
          <img
            src={`${import.meta.env.BASE_URL}logo.png`}
            alt="TapxMedia"
            className={cn(
              "object-contain shrink-0 transition-all duration-300 transform",
              collapsed ? "w-8 h-8" : "w-[160px] md:w-[200px] h-auto py-2"
            )}
          />
        </Link>
      </div>
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {!collapsed && (
                  <span className={cn(
                    "text-[10px] font-semibold whitespace-nowrap tracking-wide flex-1",
      ...
      <div className="border-t border-[#0F1E3D]/5 px-3 py-4 space-y-2">
        {!collapsed && user && (
          <div className="px-3 py-2 flex items-center gap-3 overflow-hidden">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-bold text-[#0F1E3D] truncate">{user.name}</p>
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="text-[10px] font-black uppercase tracking-widest bg-purple-100 text-purple-600 px-1.5 py-0.5 rounded-full">Client</span>
              </div>
            </div>
          </div>
        )}
        ...
        >
          ...
          {!collapsed && <span className="text-[10px] whitespace-nowrap">Log out</span>}
        </button>
      </div>
    </aside>
  );
