    ...
    <div className="flex flex-col gap-5 h-full">
      <div className="glass-card p-6 h-full flex flex-col">
        <div className="flex items-center justify-between mb-6">
           <div>
              <h3 className="text-sm font-black uppercase tracking-widest text-[#0F1E3D]">Identity Vault</h3>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
           <div className="p-4 bg-white border border-[#0F1E3D]/5 rounded-2xl">
              <span className="text-[9px] font-black uppercase text-[#0F1E3D]/40 block mb-2">Primary Palette</span>
           ...
           <div className="p-4 bg-white border border-[#0F1E3D]/5 rounded-2xl">
              <span className="text-[9px] font-black uppercase text-[#0F1E3D]/40 block mb-2">Typography</span>
              <div className="flex items-center gap-2">
                 <Type className="w-4 h-4 text-[#0F1E3D]/30" />
                 <span className="text-xs font-black text-[#0F1E3D]">DM Sans</span>
              </div>
           </div>
        </div>

        <div className="flex items-center justify-between mb-4">
           <span className="text-xs font-black uppercase text-[#0F1E3D]/80">Available Assets</span>
           {isAdmin && (
             <div>
        ...
        <div className="space-y-4 flex-1 overflow-y-auto min-h-[150px] pr-2">
           {isLoading ? (
             ...
             <div className="flex flex-col items-center justify-center h-full opacity-50 space-y-2 border-2 text-center border-dashed border-[#0F1E3D]/10 rounded-2xl py-6">
             ...
             assets.map((asset, i) => (
               <div key={asset.id || i} className="flex items-center justify-between p-4 bg-[#F8FAFC] border border-[#0F1E3D]/5 rounded-xl group transition-all hover:bg-white hover:border-blue-500/20">
                  <div className="flex items-center gap-3 overflow-hidden">
                     ...
                     <div className="overflow-hidden">
                        <span className="text-xs font-bold text-[#0F1E3D] block truncate">{formatFileName(asset.name)}</span>
                        <span className="text-[9px] font-bold text-[#0F1E3D]/30 uppercase">{getFileExtension(asset.name)} • {formatFileSize(asset.metadata?.size || 0)}</span>
                     </div>
                  </div>
