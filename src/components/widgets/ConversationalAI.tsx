...
export const ConversationalAI = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-white border border-[#0F1E3D]/10 rounded-3xl overflow-hidden shadow-sm">
      {/* Header */}
      <div className="px-6 py-4 bg-[#F8FAFC] border-b border-[#0F1E3D]/5 shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
             ...
             <div>
               <h3 className="text-xs font-black text-[#0F1E3D] uppercase tracking-widest">Growth Architect</h3>
               <div className="flex items-center gap-1.5 mt-0.5">
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                  <p className="text-[9px] text-[#0F1E3D]/40 font-black uppercase tracking-widest">AI Strategy Active</p>
               </div>
             </div>
          </div>
        </div>
      ...
      <div 
        ref={messagesEndRef}
        className="flex-1 overflow-y-auto p-6 space-y-6 bg-white scrollbar-thin scrollbar-thumb-[#0F1E3D]/10 scrollbar-track-transparent"
      >
        <AnimatePresence initial={false}>
          ...
          {isTyping && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-start gap-4">
               ...
               <div className="bg-[#F8FAFC] border border-[#0F1E3D]/5 rounded-2xl rounded-tl-none p-4 shadow-sm flex items-center gap-1.5">
      ...
      {/* Input Area */}
      <div className="p-4 bg-[#F8FAFC] border-t border-[#0F1E3D]/5 shrink-0">
        <form onSubmit={handleSubmit} className="relative flex items-center">
          <input
            ...
            disabled={isTyping}
            className="w-full bg-white border border-[#0F1E3D]/10 text-[#0F1E3D] text-xs font-medium placeholder-[#0F1E3D]/20 focus:outline-none focus:border-[#0F1E3D]/30 focus:ring-4 focus:ring-[#0F1E3D]/5 rounded-xl py-3.5 pl-4 pr-12 shadow-sm transition-all disabled:opacity-50"
          />
          <button
            type="submit"
