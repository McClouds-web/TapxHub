  ...
  return (
    <div className="glass-card p-3 h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-[10px] font-black uppercase tracking-widest text-[#0F1E3D]">Lead Pipeline</h3>
      ...
      <div className="flex-1 overflow-x-auto -mx-6 px-4">
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-[#0F1E3D]/5">
              <th className="py-3 text-[10px] font-black uppercase tracking-widest text-[#0F1E3D]/30">Contact</th>
              <th className="py-3 text-[10px] font-black uppercase tracking-widest text-[#0F1E3D]/30">Source</th>
              <th className="py-3 text-[10px] font-black uppercase tracking-widest text-[#0F1E3D]/30">Status</th>
              <th className="py-3 text-right text-[10px] font-black uppercase tracking-widest text-[#0F1E3D]/30">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[#0F1E3D]/5">
            {MOCK_LEADS.map((lead) => (
              <tr key={lead.id} className="group hover:bg-[#F8FAFC]/50 transition-all">
                <td className="py-3">
                  <p className="text-[10px] font-bold text-[#0F1E3D]">{lead.name}</p>
                ...
                <td className="py-3">
                  <div className="flex items-center gap-1.5">
                    <div className={`w-1.5 h-1.5 rounded-full ${
                      ...
                      lead.status === 'contacted' ? 'bg-amber-500' : 'bg-blue-500'
                    }`} />
      ...
      <div className="mt-4 pt-4 border-t border-[#0F1E3D]/5 flex items-center justify-between">
        <span className="text-[10px] font-bold text-[#0F1E3D]/30 uppercase tracking-widest">Showing last 4 leads</span>
        <button className="text-[10px] font-black text-blue-600 uppercase tracking-widest hover:underline">Download CSV</button>
      </div>
    </div>
  );
};
