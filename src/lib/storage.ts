...
export const uploadFile = async (file: File, bucketPath: string = 'uploads') => {
  ...
  const { data, error } = await storageClient.storage.from('vault').upload(filePath, file, { upsert: true });
  
  if (error) throw error;
  
  const { data: publicData } = storageClient.storage.from('vault').getPublicUrl(data.path);
...
export const listBrandAssets = async () => {
    const { data, error } = await storageClient.storage.from('brand_assets').list();
    if (error) {
...
export const uploadBrandAsset = async (file: File) => {
    ...
    const { data, error } = await storageClient.storage.from('brand_assets').upload(filePath, file, { upsert: false });
    if (error) throw error;
...
export const deleteBrandAsset = async (fileName: string) => {
    const { data, error } = await storageClient.storage.from('brand_assets').remove([fileName]);
    if (error) throw error;
    return data;
};

export const getBrandAssetUrl = (fileName: string) => {
    const { data } = storageClient.storage.from('brand_assets').getPublicUrl(fileName);
    return data.publicUrl;
};

export const uploadPublicFile = async (file: File, folder: 'avatars' | 'brand_assets') => {
  const targetBucket = 'vault';
  const filePath = `${folder}/${Date.now()}_${file.name.replace(/[^a-zA-Z0-9.\-_]/g, '')}`;
  
  const { data, error } = await storageClient.storage.from(targetBucket).upload(filePath, file, { upsert: true });
  
  if (error) {
    throw new Error(`Upload failed to bucket "${targetBucket}": ${error.message}`);
  }
  
  const { data: publicData } = storageClient.storage.from(targetBucket).getPublicUrl(data.path);
