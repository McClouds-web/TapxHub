    ...
    <motion.div variants={container} initial="hidden" animate="show" className="flex flex-col h-full gap-6 overflow-hidden pb-4">
      ...
      <motion.div variants={item} className="flex items-end justify-between shrink-0">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-200 rounded-xl">
          ...
          <span className="text-xs font-black uppercase tracking-widest text-amber-700">{pending} Pending</span>
        </div>
      </motion.div>
      ...
      <motion.div variants={item} className="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
        ...
        <div className="bg-white border border-[#0F1E3D]/5 rounded-2xl p-6 shadow-sm relative overflow-hidden">
        ...
        <div className="bg-white border border-[#0F1E3D]/5 rounded-2xl p-6 shadow-sm relative overflow-hidden">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600/70 mb-3 block">Confirmed</span>
          <span className="text-3xl font-extrabold text-emerald-600">{confirmed}</span>
        </div>
      </motion.div>

      {/* Booking List */}
      <motion.div variants={item} className="flex-1 bg-white border border-[#0F1E3D]/5 rounded-2xl overflow-hidden shadow-sm min-h-0 flex flex-col">
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#0F1E3D]/5 bg-[#F8FAFC]/50 shrink-0">
          <h3 className="text-sm font-black uppercase tracking-widest text-[var(--brand-primary)] flex items-center gap-2">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3">
          {isLoading ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-12">
              ...
              <p className="text-sm font-bold text-[var(--brand-primary)]">No bookings yet.</p>
              <p className="text-xs text-[var(--brand-primary)]/40 mt-1">Bookings from TapxMedia will appear here automatically.</p>
            </div>
          ) : (
            <div className="space-y-3 py-2">
              <AnimatePresence>
                {bookings.map((booking) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="flex items-center justify-between p-5 rounded-xl border border-[#0F1E3D]/5 hover:border-[#0F1E3D]/10 hover:bg-[#F8FAFC] transition-all group cursor-pointer"
                    >
                      {/* Avatar + Info */}
                      <div className="flex items-center gap-4 min-w-0">
                        <div className="w-10 h-10 rounded-lg bg-[#0F1E3D] flex items-center justify-center text-white text-xs font-black shrink-0">
                          {booking.name.charAt(0)}
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-extrabold text-[var(--brand-primary)] truncate">{booking.name}</p>
                      ...
                      {/* Date + Time */}
                      <div className="hidden md:flex items-center gap-6 shrink-0 mx-8">
                      ...
                      <div className="flex items-center gap-3 shrink-0" onClick={(e) => e.stopPropagation()}>
                        <span className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border", status.color)}>
                        ...
                        {booking.status === "pending" && (
                          <button
                            ...
                            className="px-3 py-1.5 bg-[#0F1E3D] text-white rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-[#1E3A8A] transition-all shadow-sm disabled:opacity-60"
                          >
                            Confirm
                          </button>
                        )}
        ...
        {selected && !showConvert && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-[#0F1E3D]/5 overflow-hidden"
            >
              {/* Modal header */}
              <div className="flex items-center justify-between px-8 py-6 border-b border-[#0F1E3D]/5 bg-[#F8FAFC]/50">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl bg-[#0F1E3D] flex items-center justify-center text-white text-lg font-extrabold shadow-md">
                    {selected.name.charAt(0)}
                  </div>
                  <div>
                    <h2 className="text-xl font-extrabold text-[var(--brand-primary)]">{selected.name}</h2>
                    <span className={cn(
              ...
              <div className="px-8 py-6 space-y-4">
                ...
                {selected.message && (
                  <div className="bg-[#F8FAFC] border border-[#0F1E3D]/5 rounded-xl p-4">
                    <p className="text-[10px] font-black uppercase tracking-widest text-[var(--brand-primary)]/40 mb-2">Message</p>
                    <p className="text-sm font-medium text-[var(--brand-primary)] leading-relaxed">{selected.message}</p>
                  </div>
                )}
              </div>

              {/* Modal footer actions */}
              <div className="px-8 py-5 border-t border-[#0F1E3D]/5 bg-[#F8FAFC]/30 flex flex-wrap gap-3">
                {selected.status === "pending" && (
                  <button
                    onClick={() => { updateBooking.mutate({ id: selected.id, status: "confirmed" }); setSelected(null); }}
                    disabled={updateBooking.isPending}
                    className="flex items-center gap-1.5 px-4 py-2.5 bg-emerald-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-sm"
                  >
                ...
                {selected.status === "pending" && (
                  <button
                    ...
                    disabled={updateBooking.isPending}
                    className="flex items-center gap-1.5 px-4 py-2.5 bg-red-50 text-red-600 border border-red-200 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-red-100 transition-all"
                  >
                ...
                {["confirmed", "pending"].includes(selected.status) && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-2.5 bg-[#0F1E3D] text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-[#1E3A8A] transition-all shadow-md"
                  >
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-xl font-extrabold text-[var(--brand-primary)]">Convert to Client</h2>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-[#F8FAFC] border border-[#0F1E3D]/5 rounded-xl p-4 text-xs font-medium text-[var(--brand-primary)]/60 leading-relaxed">
...
function InfoRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="bg-[#F8FAFC] border border-[#0F1E3D]/5 rounded-xl p-3.5">
      ...
      <p className="text-xs font-bold text-[var(--brand-primary)] truncate">{value}</p>
    </div>
  );
}
