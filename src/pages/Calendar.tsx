  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[13px] font-bold tracking-tight text-foreground">Calendar</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-semibold text-foreground min-w-[130px] text-center">{monthLabel}</span>
          <button
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {summary.map((s) => (
          <div key={s.label} className="glass-card p-3 text-center">
            <p className={cn("text-[13px] font-bold", s.color)}>{s.count}</p>
      ...
      {!isAdmin && (
        <div className="glass-card p-3 flex items-center gap-4 border-dashed">
          <div className="w-10 h-10 rounded-xl bg-accent/10 flex items-center justify-center shrink-0">
            <CalendarCheck className="w-5 h-5 text-accent" />
          </div>
          <div>
            <p className="text-[10px] font-bold text-foreground">Want to book a session?</p>
            <p className="text-[10px] text-muted-foreground">Visit the TapxMedia website to schedule a consultation with Tapiwa.</p>
          </div>
          <a
            href="https://www.tapxmedia.com/#booking"
            target="_blank"
            rel="noopener noreferrer"
            className="ml-auto shrink-0 px-4 py-2 bg-[#0F1E3D] text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#1E3A8A] transition-all shadow-md"
          >
            Book Now
          </a>
        </div>
