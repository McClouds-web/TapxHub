    ...
    <motion.div variants={container} initial="hidden" animate="show"
      ...
      <motion.div variants={item} className="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
        ...
        <div className="bg-white border border-[#0F1E3D]/5 rounded-2xl p-6 shadow-sm flex flex-col justify-between relative overflow-hidden">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600/70 mb-3">Active Engagements</span>
          <span className="text-3xl font-extrabold text-emerald-600 tracking-tight">{activeCount}</span>
        </div>
        <div className="bg-white border border-[#0F1E3D]/5 rounded-2xl p-6 shadow-sm flex flex-col justify-between relative overflow-hidden">
      ...
      <motion.div variants={item} className="flex gap-5 flex-1 min-h-0">
        ...
        {/* Client List */}
        <div className="flex-1 flex flex-col bg-white border border-[#0F1E3D]/5 rounded-2xl overflow-hidden shadow-sm min-h-0">
          <div className="flex items-center justify-between px-6 py-5 border-b border-[#0F1E3D]/5 shrink-0 bg-[#F8FAFC]/50 gap-4">
            <h3 className="text-sm font-black uppercase tracking-widest text-[var(--brand-primary)] flex items-center gap-2 shrink-0">
            ...
            <div className="relative max-w-xs w-full">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)}
                ...
                className="w-full bg-white border border-[#0F1E3D]/10 rounded-xl pl-9 pr-4 py-2.5 text-xs font-bold text-[var(--brand-primary)] placeholder:text-[#0F1E3D]/30 focus:outline-none focus:border-[var(--brand-accent)] transition-colors shadow-sm"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-2 py-2 relative">
            {isLoading ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-center p-8">
                <div className="w-16 h-16 rounded-2xl bg-[#F8FAFC] border border-[#0F1E3D]/5 flex items-center justify-center mb-4">
                  <Users className="h-6 w-6 text-[var(--brand-primary)] opacity-30" />
                </div>
                <p className="text-sm font-bold text-[var(--brand-primary)]">No clients found.</p>
                <button onClick={openAdd} className="mt-3 px-4 py-2 bg-[#0F1E3D] text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-[#1E3A8A] transition-all">
                  Add First Client
                </button>
              </div>
            ) : (
                  ...
                  {filtered.map((client, i) => {
                    ...
                    return (
                      <motion.div key={client.id} layout
                        ...
                        className={cn(
                          "flex items-center justify-between p-4 rounded-xl group transition-all cursor-pointer border relative",
                          isSelected
                      ...
                      >
                        <div className="flex items-center gap-4 min-w-0">
                          <div className={cn(
                            "w-10 h-10 rounded-lg border flex items-center justify-center shrink-0 transition-colors shadow-sm",
                            ...
                            <span className="text-xs font-black uppercase">{client.name.charAt(0)}</span>
                          </div>
                          <div className="min-w-0">
                            <p className="text-sm font-extrabold text-[var(--brand-primary)] truncate">{client.name}</p>
                              ...
                              {openMenuId === client.id && (
                                ...
                                >
                                  <button
                                    ...
                                    className="w-full px-4 py-2 text-left text-xs font-bold text-[var(--brand-primary)] hover:bg-[#F8FAFC] transition-colors flex items-center gap-2"
                                  >
        ...
        <AnimatePresence>
          {selected && (
            <motion.div
              ...
              className="hidden shrink-0 lg:flex flex-col h-full overflow-y-auto relative bg-white border border-[#0F1E3D]/5 rounded-2xl shadow-sm"
            >
              <div className="p-6 space-y-6">
                <div className="flex items-start justify-between">
                  <div className="flex gap-4">
                    <div className="w-14 h-14 rounded-xl bg-[#0F1E3D] flex items-center justify-center text-white text-xl font-extrabold shadow-md shrink-0 border border-[#1E3A8A]">
                      {selected.name.charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <h3 className="text-xl font-extrabold text-[var(--brand-primary)] leading-tight">{selected.name}</h3>
                      {selected.industry && (
                        <span className="inline-block mt-1.5 px-2.5 py-1 bg-[#F8FAFC] border border-[#0F1E3D]/5 rounded-lg text-[9px] font-black uppercase tracking-widest text-[#1E3A8A]">
                          {selected.industry}
                        </span>
                      )}
                      <div className="mt-1.5">
                        <span className={cn(
                          "inline-flex items-center px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border",
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-[#F8FAFC] rounded-xl p-4 border border-[#0F1E3D]/5">
                    <span className="text-[9px] font-black uppercase tracking-widest text-[#0F1E3D]/40 mb-1 block">Type</span>
                    <span className="text-sm font-extrabold text-[#0F1E3D] block capitalize">
                      {selected.client_type ?? "—"}
                    </span>
                  </div>
                  <div className="bg-[#F8FAFC] rounded-xl p-4 border border-[#0F1E3D]/5">
                    <span className="text-[9px] font-black uppercase tracking-widest text-[#0F1E3D]/40 mb-1 block">Retainer</span>
                    <span className="text-sm font-extrabold text-[#0F1E3D] block">
                    ...
                    {(selected.contact_email || selected.email) && (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs font-bold text-[#0F1E3D] truncate">{selected.contact_email ?? selected.email}</p>
                        </div>
                      </a>
                    )}
                    {selected.phone && (
                      <div className="w-full flex items-center gap-3 p-3 rounded-xl border border-transparent hover:border-[#0F1E3D]/5 hover:bg-[#F8FAFC] group transition-all cursor-pointer">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs font-bold text-[#0F1E3D] truncate">{selected.phone}</p>
                        </div>
                      </div>
                    )}
                    {selected.site && (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs font-bold text-[#1E3A8A] truncate hover:underline">{selected.site}</p>
                        </div>
                      </a>
                    )}
                  </div>
                ...
                {selected.onboarding_completed && selected.business_profile ? (
                    ...
                    <div className="space-y-3">
                       {selected.business_profile.industry && (
                          <div className="bg-[#F8FAFC] rounded-xl p-3 border border-[#0F1E3D]/5">
                             <p className="text-[9px] font-black uppercase tracking-widest text-[var(--brand-primary)]/40 mb-1 pointer-events-none">Industry & Target</p>
                             <p className="text-xs font-bold text-[#0F1E3D]">{selected.business_profile.industry} • {selected.business_profile.targetAudience || 'Unknown Audience'}</p>
                          </div>
                       )}
                       {selected.business_profile.biggestChallenge && (
                          <div className="bg-red-50/50 rounded-xl p-3 border border-red-100">
                             <p className="text-[9px] font-black uppercase tracking-widest text-red-800/60 mb-1 pointer-events-none">Biggest Challenge</p>
                             <p className="text-xs font-bold text-red-900">{selected.business_profile.biggestChallenge}</p>
                          </div>
                       )}
                       {selected.business_profile.primaryGoal && (
                          <div className="bg-emerald-50/50 rounded-xl p-3 border border-emerald-100">
                             <p className="text-[9px] font-black uppercase tracking-widest text-emerald-800/60 mb-1 pointer-events-none">Primary Goal</p>
                             <p className="text-xs font-bold text-emerald-900">{selected.business_profile.primaryGoal}</p>
                          </div>
                       )}
                       ...
                       <div className="grid grid-cols-2 gap-2 mt-2">
                           {selected.business_profile.runPaidAds && (
                              <div className="bg-[#F8FAFC] rounded-lg p-2.5 border border-[#0F1E3D]/5">
                                 <p className="text-[8px] font-black uppercase tracking-widest text-[var(--brand-primary)]/40">Running Ads</p>
                           ...
                           {selected.business_profile.activeSocials?.length > 0 && (
                              <div className="bg-[#F8FAFC] rounded-lg p-2.5 border border-[#0F1E3D]/5">
                                 <p className="text-[8px] font-black uppercase tracking-widest text-[var(--brand-primary)]/40">Social Platforms</p>
                           ...
                           {selected.business_profile.collectEmails === "Yes" && (
                              <div className="bg-[#F8FAFC] rounded-lg p-2.5 border border-[#0F1E3D]/5">
                                 <p className="text-[8px] font-black uppercase tracking-widest text-[var(--brand-primary)]/40">Email List</p>
                           ...
                           {selected.business_profile.hasBlog && (
                              <div className="bg-[#F8FAFC] rounded-lg p-2.5 border border-[#0F1E3D]/5">
                                 <p className="text-[8px] font-black uppercase tracking-widest text-[var(--brand-primary)]/40">Active Blog</p>
                  ...
                  <div className="border-t border-[#0F1E3D]/5 pt-5 pb-2">
                    <div className="bg-[#F8FAFC] rounded-xl p-4 border border-[#0F1E3D]/10 border-dashed flex flex-col items-center justify-center text-center">
                       <Sparkles className="h-5 w-5 text-[var(--brand-primary)]/20 mb-2" />
                       <p className="text-xs font-bold text-[var(--brand-primary)]/60">No Discovery Brief</p>
                       <p className="text-[9px] font-bold text-[var(--brand-primary)]/30 uppercase tracking-widest mt-0.5">Client has not completed onboarding.</p>
                    </div>
                  </div>
                )}
                ...
                <div className="pt-4 flex items-center gap-3 border-t border-[#0F1E3D]/5">
                  <button
                    ...
                    className="flex-1 px-4 py-2.5 bg-[#0F1E3D] text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-[#1E3A8A] transition-all shadow-md active:scale-95 flex items-center justify-center gap-1.5 disabled:opacity-60"
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 bg-white border border-[#0F1E3D]/10 text-[#0F1E3D] hover:bg-[#F8FAFC] rounded-xl text-xs font-bold uppercase tracking-widest transition-all shadow-sm active:scale-95 flex items-center gap-1.5"
                  >
        ...
        {modalMode !== null && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg border border-[#0F1E3D]/5 max-h-[90vh] overflow-y-auto"
            >
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-xl font-extrabold text-[var(--brand-primary)]">
                    {modalMode === "add" ? "Add New Client" : "Edit Client"}
                  </h2>
                  <p className="text-xs font-bold uppercase tracking-widest text-[var(--brand-primary)]/40 mt-0.5">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-xs font-black uppercase tracking-widest text-[var(--brand-primary)]/50 mb-1.5 block">Client Type</label>
                  <select
...
function Field({
  ...
  return (
    <div>
      <label className="text-xs font-black uppercase tracking-widest text-[var(--brand-primary)]/50 mb-1.5 block">{label}</label>
      <input
        type={type}
        value={value}
