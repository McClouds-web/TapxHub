...
export default function Media() {
    ...
    return (
        <motion.div variants={container} initial="hidden" animate="show" className="space-y-5 pb-12 overflow-x-hidden">
            <motion.div variants={item} className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p className="text-[10px] text-[#0F1E3D]/50 font-bold uppercase tracking-widest mt-1">
                ...
                <div className="flex items-center gap-3">
                    <div className="relative group">
                        ...
                        <input 
                            ...
                            className="bg-white border border-[#0F1E3D]/5 rounded-xl pl-10 pr-4 py-2.5 text-[10px] font-bold text-[#0F1E3D] focus:outline-none focus:border-[#1E3A8A] focus:shadow-md transition-all w-64 shadow-sm"
                        />
                    </div>
                    {isAdmin && (
                        <Button 
                            ...
                            className={cn(
                                "rounded-xl px-4 py-2.5 text-[10px] font-black uppercase tracking-widest transition-all shadow-lg active:scale-95",
            ...
            <AnimatePresence>
                {isUploading && (
                    ...
                    >
                        <form onSubmit={handleUpload} className="bg-white border border-[#0F1E3D]/5 p-8 rounded-[2.5rem] shadow-2xl flex flex-col md:flex-row gap-4 items-end">
                            <div className="flex-1 w-full space-y-4">
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-[#0F1E3D]/40 ml-2">Secure File</label>
                                    <div className="relative border-2 border-dashed border-[#0F1E3D]/10 bg-[#F8FAFC] rounded-xl p-3 flex items-center justify-center group hover:border-[#1E3A8A]/30 transition-all cursor-pointer">
                                        ...
                                        <div className="flex flex-col items-center gap-1.5 py-4">
                                            ...
                                            <span className="text-[10px] font-bold text-[#0F1E3D]/60">{uploadForm.file ? uploadForm.file.name : "Drop file here or browse"}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="w-full md:w-64 space-y-4">
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black uppercase tracking-widest text-[#0F1E3D]/40 ml-2">Assign to Client</label>
                                    <select 
                                        className="w-full bg-[#F8FAFC] border border-[#0F1E3D]/10 rounded-xl px-4 py-3.5 text-[10px] font-bold text-[#0F1E3D] focus:outline-none focus:border-[#1E3A8A] transition-all"
                            ...
                            <button 
                                ...
                                className="w-full md:w-auto px-6 py-4 bg-[#0F1E3D] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1E3A8A] transition-all shadow-xl active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
                            >
            ...
            <motion.div variants={container} className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                <AnimatePresence mode="popLayout">
                    {filteredFiles.map((file) => (
                        <motion.div
                            ...
                            variants={item}
                            whileHover={{ y: -8 }}
                            className="bg-white group flex flex-col p-3 rounded-[2rem] border border-[#0F1E3D]/5 shadow-sm hover:shadow-xl transition-all duration-300 relative overflow-hidden"
                        >
                            ...
                            <div className="flex items-start justify-between gap-2 px-1">
                                <div className="min-w-0 flex-1">
                                    <p className="truncate text-[10px] font-black text-[#0F1E3D] leading-tight group-hover:text-[#1E3A8A] transition-colors uppercase tracking-wider">
                                        {file.name}
                                    </p>
                                    <div className="flex items-center gap-2 mt-2">
                                        <span className="text-[10px] font-black uppercase text-[#0F1E3D]/40 tracking-widest px-2 py-0.5 bg-[#F8FAFC] rounded-lg border border-[#0F1E3D]/5">
                                            {file.file_size_mb} MB
                                        </span>
                                        <span className="text-[10px] font-bold text-[#0F1E3D]/30 uppercase tracking-tight">
                    ...
                    {filteredFiles.length === 0 && !filesLoading && (
                         <div className="col-span-full py-20 flex flex-col items-center justify-center opacity-20">
                             ...
                             <p className="font-black uppercase tracking-[0.2em] text-[10px]">No Assets Found</p>
                         </div>
                    )}
                </AnimatePresence>
            </motion.div>
