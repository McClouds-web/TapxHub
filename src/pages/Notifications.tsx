...
function getTypeInfo(type?: string): { icon: React.ElementType; color: string; bg: string; label: string } {
  switch (type) {
    case "task":    return { icon: CheckCircle2, color: "#3b82f6", bg: "bg-blue-500/10", label: "Task" };
    ...
    <motion.div variants={container} initial="hidden" animate="show"
      ...
      <motion.div variants={item} className="flex items-end justify-between shrink-0">
        ...
        {unread > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2.5 bg-[#0F1E3D] text-white rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-[#1E3A8A] transition-all shadow-md active:scale-95 disabled:opacity-50"
          >
      ...
      <motion.div variants={item} className="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
        ...
        <div className="bg-white border border-[#0F1E3D]/5 rounded-xl p-3 shadow-sm flex flex-col justify-between relative overflow-hidden">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-blue-600/70 mb-3">Read</span>
          <span className="text-3xl font-extrabold text-blue-600 tracking-tight">{notifications.length - unread}</span>
        </div>
      </motion.div>
      ...
      <motion.div variants={item} className="flex-1 min-h-0 bg-white border border-[#0F1E3D]/5 rounded-xl shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-4 border-b border-[#0F1E3D]/5 shrink-0 bg-[#F8FAFC]/50 gap-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-[var(--brand-primary)]">Inbox</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2">
          {isLoading ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-8">
              ...
              <p className="text-[10px] font-bold text-[var(--brand-primary)]">No notifications here.</p>
            </div>
          ) : (
                ...
                {visible.map((notif, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={cn(
                        "flex items-start gap-4 p-3 rounded-xl transition-colors cursor-pointer border border-transparent hover:border-[#0F1E3D]/5 hover:bg-[#F8FAFC]",
