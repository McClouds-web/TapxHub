...
export default function Planner() {
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="flex flex-col h-full gap-6 overflow-hidden">
      ...
      <motion.div variants={item} className="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
        <div>
          ...
          <p className="text-sm font-bold text-[var(--brand-primary)]/50 uppercase tracking-widest mt-1.5 flex items-center gap-2">
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            ...
            <input 
              ...
              className="w-full sm:w-64 pl-9 pr-4 py-2.5 bg-white border border-[#0F1E3D]/10 rounded-xl text-xs font-bold text-[#0F1E3D] placeholder:text-[#0F1E3D]/30 focus:outline-none focus:border-[#1E3A8A] shadow-sm transition-all"
            />
          </div>
          <button 
            onClick={() => setIsAdding(!isAdding)}
            className="px-5 py-3 bg-[#0F1E3D] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1E3A8A] transition shadow-md flex items-center gap-2 whitespace-nowrap active:scale-95"
          >
      ...
      <div className="flex-1 flex flex-col xl:flex-row gap-6 min-h-0">
        ...
        <motion.div variants={item} className="xl:w-[320px] flex flex-col shrink-0 gap-6 min-h-0">
           <div className="bg-white rounded-3xl p-6 shadow-sm border border-[#0F1E3D]/5 flex flex-col min-h-0">
              <div className="flex items-center justify-between mb-4">
                 ...
                 {selectedDate && (
                    <button onClick={() => setSelectedDate(undefined)} className="text-[9px] font-black uppercase text-[#1E3A8A]/50 hover:text-[#1E3A8A] transition">Show All</button>
                 )}
              </div>
              ...
              <Calendar
                 ...
                 className="w-full bg-[#F8FAFC]/50 rounded-2xl p-2 border border-[#0F1E3D]/5 shadow-inner"
                 classNames={{
              ...
              <div className="mt-8 flex-1 overflow-y-auto min-h-0 pb-4 no-scrollbar">
                 ...
                 {dailyMeetings.length === 0 ? (
                    <div className="p-6 bg-[#F8FAFC] rounded-2xl border border-dashed border-[#0F1E3D]/10 text-center">
                    ...
                    <div className="space-y-3">
                       {dailyMeetings.map(m => (
                          <div key={m.id} className={cn(
                            "group p-3.5 bg-[#F8FAFC]/80 border rounded-2xl transition-all hover:shadow-sm relative",
                            m.status === 'completed' ? "border-emerald-500/20 opacity-60" : "border-[#0F1E3D]/5 hover:border-[#1E3A8A]/20"
                          )}>
                             <div className="flex items-center justify-between mb-1.5">
                                <span className={cn("text-[9px] font-black uppercase tracking-widest", m.status === 'completed' ? "text-emerald-600" : "text-[#1E3A8A]")}>
                                  {m.startTime} — {m.endTime}
                                </span>
                                <div className="flex items-center gap-1">
                                   <button 
                                     onClick={() => toggleMeetingStatus(m)}
                                     className={cn("p-1.5 rounded-lg transition-colors", m.status === 'completed' ? "bg-emerald-500 text-white" : "bg-white text-[#0F1E3D]/10 hover:text-emerald-500")}
                                   >
                             ...
                             <p className={cn("text-xs font-bold text-[#0F1E3D] leading-tight mb-2", m.status === 'completed' && "line-through")}>{m.title}</p>
                             {m.company_id && (
                                <span className="inline-flex items-center gap-1 text-[8px] font-black uppercase tracking-widest px-2 py-0.5 bg-white border border-[#0F1E3D]/5 rounded-md text-[#0F1E3D]/40">
        ...
        <div className="flex-1 flex flex-col min-h-0 gap-6">
              ...
              >
                <div className="flex items-center justify-between mb-6">
                   <div className="flex items-center gap-3">
                      ...
                      <div>
                         <h4 className="text-xl font-extrabold text-[var(--brand-primary)] m-0">Create Strategy Item</h4>
                   ...
                   <div className="flex bg-[#F8FAFC] p-1 rounded-xl border border-[#0F1E3D]/5">
                      <button type="button" onClick={() => setAddMode('task')} className={cn("px-4 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all", addMode === 'task' ? "bg-white text-[#1E3A8A] shadow-sm" : "text-[#0F1E3D]/30")}>Task</button>
                      <button type="button" onClick={() => setAddMode('meeting')} className={cn("px-4 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all", addMode === 'meeting' ? "bg-white text-[#1E3A8A] shadow-sm" : "text-[#0F1E3D]/30")}>Meeting</button>
                   </div>
                </div>

                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                   <div className="space-y-1.5">
                      <label className="text-[9px] uppercase font-black tracking-widest text-[#0F1E3D]/40 ml-2">Name / Objective</label>
                      <input 
                        autoFocus
                        value={form.title}
                   ...
                   {addMode === 'task' ? (
                      <div className="space-y-1.5">
                         <label className="text-[9px] uppercase font-black tracking-widest text-[#0F1E3D]/40 ml-2">Client Reference</label>
                         <input 
                           value={form.tag}
                      ...
                      <>
                         <div className="space-y-1.5">
                            <label className="text-[9px] uppercase font-black tracking-widest text-[#0F1E3D]/40 ml-2">Link to Company</label>
                            <select 
                         ...
                         <div className="space-y-1.5">
                            <label className="text-[9px] uppercase font-black tracking-widest text-[#0F1E3D]/40 ml-2">Duration Slot</label>
                            <div className="flex items-center gap-2">
                               <input type="time" value={form.startTime} onChange={e => setForm({ ...form, startTime: e.target.value })} className="flex-1 bg-[#F8FAFC] border border-[#0F1E3D]/10 rounded-xl px-3 py-3 text-sm font-bold" />
                               <span className="text-[#0F1E3D]/20">—</span>
                               <input type="time" value={form.endTime} onChange={e => setForm({ ...form, endTime: e.target.value })} className="flex-1 bg-[#F8FAFC] border border-[#0F1E3D]/10 rounded-xl px-3 py-3 text-sm font-bold" />
                            </div>
                         </div>
                      </>
                   )}
                ...
                <div className="flex justify-between items-center pt-6 mt-6 border-t border-[#0F1E3D]/5">
                   {addMode === 'meeting' ? (
                   ...
                   <button 
                     ...
                     className="px-8 py-3.5 bg-[#0F1E3D] text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#1E3A8A] transition-all shadow-lg active:scale-95 disabled:opacity-50 flex items-center gap-2"
                   >
          ...
          {/* Kanban Flow */}
          <motion.div variants={item} className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-6 min-h-0">
...
function KanbanColumn({ title, icon, tasks, count, color, active, onMove, onDelete, prevStatus, nextStatus }: any) {
  return (
    <div className={cn(
       ...
       <div className={cn("p-6 border-b border-[#0F1E3D]/5 flex items-center justify-between shrink-0", active ? "bg-[#1E3A8A]/[0.02]" : "bg-transparent")}>
         <h3 className={cn("text-xs font-black uppercase tracking-widest flex items-center gap-2", active ? "text-[#1E3A8A]" : "text-[var(--brand-primary)]/40")}>
           {icon} {title}
         </h3>
...
function TaskCard({ task, onMove, onDelete, prevStatus, nextStatus }: any) {
  ...
  return (
    <motion.div 
      ...
      className={cn(
        "bg-white p-5 rounded-3xl border shadow-sm group transition-all duration-300 relative overflow-hidden",
        isDone ? "border-emerald-500/10 opacity-70 bg-emerald-50/20" : "border-[#0F1E3D]/5 hover:border-[#1E3A8A]/30 hover:shadow-lg"
      )}
    >
      <div className="flex items-start justify-between gap-3 mb-4">
         <p className={cn("text-sm font-extrabold text-[#0F1E3D] leading-tight", isDone && "line-through opacity-50")}>{task.title}</p>
      ...
      <div className="flex items-center justify-between">
         <div className="flex items-center gap-2">
            {task.company_name && (
               <span className="px-2 py-0.5 rounded-lg border border-[#0F1E3D]/10 bg-[#F8FAFC] text-[8px] font-black uppercase tracking-widest text-[#1E3A8A]">
                  {task.company_name}
               </span>
            )}
            <span className="text-[9px] font-bold text-[#0F1E3D]/30 uppercase tracking-widest">{dateStr}</span>
         </div>
