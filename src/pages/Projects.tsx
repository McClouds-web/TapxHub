            ...
            <div key={col.key} className="min-w-[300px] flex-1 snap-center">
              <div className={cn("mb-4 flex items-center justify-between border-b-2 pb-3", col.color)}>
                <div className="flex items-center gap-2">
                  <h3 className="text-[10px] font-bold uppercase tracking-wider text-foreground">{col.label}</h3>
                  <span className="rounded-full bg-white/40 border border-white/60 px-2.5 py-0.5 text-[10px] font-bold text-muted-foreground backdrop-blur-md">
                    {colProjects.length}
                  </span>
                </div>
              </div>
                ...
                <AnimatePresence>
                  {colProjects.map((project) => (
                    <motion.div
                      ...
                      whileTap={{ scale: 0.98 }}
                      className={cn(
                        "glass-card cursor-pointer p-3 transition-all group relative overflow-hidden",
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-3">
                        <div>
                          <p className="text-[10px] font-bold text-foreground leading-tight group-hover:text-accent transition-colors">
                            {project.title}
                          </p>
                          {isAdmin && (
