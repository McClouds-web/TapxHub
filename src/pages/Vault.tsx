    ...
    <motion.div variants={container} initial="hidden" animate="show"
      ...
      <motion.div variants={item} className="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
        ...
        <div className="bg-white border border-[#0F1E3D]/5 rounded-xl p-3 shadow-sm flex flex-col justify-between relative overflow-hidden">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-blue-600/70 mb-3">Deliverables</span>
          <div className="flex items-end gap-2">
            <span className="text-3xl font-extrabold text-blue-600 tracking-tight">{secureFiles}</span>
      ...
      <motion.div variants={item} className="flex-1 min-h-0 bg-white border border-[#0F1E3D]/5 rounded-xl shadow-sm overflow-hidden flex flex-col">
        {/* Toolbar */}
        <div className="flex items-center justify-between px-4 py-4 border-b border-[#0F1E3D]/5 shrink-0 bg-[#F8FAFC]/50 gap-4 flex-wrap">
          <div className="flex items-center gap-2 shrink-0">
            <FolderLock className="h-4 w-4 text-[#1E3A8A]" />
            <span className="text-[10px] font-black uppercase tracking-widest text-[var(--brand-primary)]">Files</span>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="relative">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)}
                ...
                className="w-44 bg-white border border-[#0F1E3D]/10 rounded-xl pl-9 pr-3 py-2 text-[10px] font-bold text-[var(--brand-primary)] placeholder:text-[#0F1E3D]/30 focus:outline-none focus:border-[var(--brand-accent)] transition-colors"
              />
            </div>
          </div>
        </div>
        ...
        {/* Table head */}
        <div className="grid grid-cols-[auto_1fr_auto_auto_auto] gap-4 items-center px-4 py-2.5 border-b border-[#0F1E3D]/5 bg-[#F8FAFC]/30 shrink-0">
          <div className="w-9" />
          <span className="text-[10px] font-black uppercase tracking-widest text-[var(--brand-primary)]/30">Name</span>
          <span className="text-[10px] font-black uppercase tracking-widest text-[var(--brand-primary)]/30 hidden md:block">Client</span>
          <span className="text-[10px] font-black uppercase tracking-widest text-[var(--brand-primary)]/30 hidden sm:block">Size</span>
          <span className="text-[10px] font-black uppercase tracking-widest text-[var(--brand-primary)]/30 w-16 text-right">Actions</span>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="h-full flex flex-col items-center justify-center p-8">
              ...
              {files.length === 0 && (
                <button
                  onClick={() => setShowUpload(true)}
                  className="mt-3 px-4 py-2 bg-[#0F1E3D] text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#1E3A8A] transition-all"
                >
                  Upload First File
                </button>
              )}
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="text-[10px] font-bold text-[var(--brand-primary)] truncate">{file.name}</p>
                          {file.is_deliverable && (
                      ...
                      {/* Client */}
                      <span className="hidden md:block text-[10px] font-bold text-[var(--brand-primary)]/40 truncate max-w-[120px]">
                        {file.company_name ?? "—"}
                      </span>

                      {/* Size */}
                      <span className="hidden sm:block text-[10px] font-bold text-[var(--brand-primary)]/50 tabular-nums w-16 text-right">
                          ...
                          {openMenuId === file.id && (
                            ...
                            >
                              <button
                                ...
                                className="w-full px-4 py-2 text-left text-[10px] font-bold text-[var(--brand-primary)] hover:bg-[#F8FAFC] transition-colors"
                              >
                                Download
                              </button>
                              <button
                                ...
                                className="w-full px-4 py-2 text-left text-[10px] font-bold text-red-600 hover:bg-red-50 transition-colors"
                              >
                                Delete
                              </button>
                            </motion.div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-[12px] font-extrabold text-[var(--brand-primary)]">Upload File</h2>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--brand-primary)]/40 mt-0.5">Add to Vault</p>
                </div>
                  ...
                  >
                    {selectedFile ? (
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-[#1E3A8A] truncate max-w-[200px]">{selectedFile.name}</span>
                      </div>
                    ) : (
                      <>
                        <Upload className="h-5 w-5 text-[var(--brand-primary)]/30 mx-auto mb-2" />
                        <p className="text-[10px] font-bold text-[var(--brand-primary)]/50">Click to select a file</p>
